...
<jsp:include page="../navBarStaff.jsp"/> 
    <div class="body__rooms">
    	...
    	%>
    	<form class="dp-n form-container" method="get" action="<%= url.urlServer + "selectRoom" %>">
    		...
    		<input type="text" name="maxAdult" id="maxAdult">
    		<input type="text" name="maxChild" id="maxChild">
            ...
            <div date-rangepicker class="flex items-center option__filter">
                <div class="relative">
                    ...
                    <%
                    	...
                    	String timeNowStr = String.format("%02d/%02d/%d", timeNow.getMonthValue(), timeNow.getDayOfMonth(), timeNow.getYear());
                    	LocalDate timeNowNext = LocalDate.now().plusDays(1);
                    	String timeNowStrNext = String.format("%02d/%02d/%d", timeNowNext.getMonthValue(), timeNowNext.getDayOfMonth(), timeNowNext.getYear());
                    %>
                    <input readonly name="start" type="text"
                        ...
                        placeholder="Check in" id="timeStartValue" value="<%= request.getAttribute("timeStart") != null ? request.getAttribute("timeStart") : timeNowStr %>" required>
                </div>
                <div class="relative">
                    <div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                        <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true"
                    ...
                    <input readonly name="end" type="text"
                        ...
                        placeholder="Check out" id="timeEndValue" value="<%= request.getAttribute("timeEnd") != null ? request.getAttribute("timeEnd") : timeNowStrNext %>" required>
                </div>
                <div class="relative">
                ...
                <button class="button_search" type="button" onclick="submitFindRoom()">
                    <i class="fas fa-search"></i>
                </button>
            </div>
            <div class="option__filter">
                 <%
                	if(listAllTypeRoom != null){
	...
	<div style="margin-top: 50px" class="room">
        <div class="room-list unbooked">
            <h2>Phòng chưa đặt</h2>
            <table>
                <thead>
                    <tr>
                        <th>Loại phòng</th>
                ...
                <%
                	...
                	if(listRoom != null){
                		for(int i = 0; i < listRoom.size(); ++i){
                			TypeRoom typeRoom = TypeRoomDAOimpl.getInstance().selectByID(listRoom.get(i).getIDTypeRoom());
                			%>
                			<tr>
                        		<td><%= typeRoom != null ? typeRoom.getTypeRoomName() : "" %></td>
                        		<td><%= listRoom.get(i).getRoomName() %></td>
                        		<td><%= typeRoom != null ? typeRoom.getPrice() : "" %></td>
                        		<td><%= typeRoom != null ? typeRoom.getMaxAdult() : "" %></td>
                        		<td><%= typeRoom != null ? typeRoom.getMaxChild() : "" %></td>
                        		<td><%= typeRoom != null ? typeRoom.getPrice() * 20 / 100 : "" %></td>
                        		<td><%= typeRoom != null ? typeRoom.getPrice() * 10 / 100 : "" %></td>
                        		<td><button class="book-button" onclick="formSubmitClick('<%= typeRoom.getTypeRoomName() %>','<%= listRoom.get(i).getIDRoom() %>','<%= listRoom.get(i).getRoomName() %>',<%= typeRoom.getPrice() %>, <%= typeRoom.getMaxAdult() %>,<%= typeRoom.getMaxChild() %>)">Đặt phòng</button></td>
                    		</tr>
                			<%
                		}
                	}
...
function unBooked(){
	document.querySelector('.btnUnBooked').classList.toggle('bg-green');
	document.querySelector('.unbooked').classList.toggle('dp-n');
}

function booked(){
	document.querySelector('.btnBooked').classList.toggle('bg-green');
	document.querySelector('.bookedCheckIn').classList.toggle('dp-n');
}

function nonCheckIn(){
	document.querySelector('.btnNonCheckIn').classList.toggle('bg-green');
	document.querySelector('.unCheckIn').classList.toggle('dp-n');
}
...
const timeStart = document.getElementById('ComeInDate');
const timeEnd = document.getElementById('ComeOutDate');

const strDateIn = document.getElementById('timeStartValue').value;
const strDateOut = document.getElementById('timeEndValue').value;

timeStart.value = strDateIn.substring(6,10) + "-" + strDateIn.substring(0,2) + "-" + strDateIn.substring(3,5);
timeEnd.value = strDateOut.substring(6,10) + "-" + strDateOut.substring(0,2) + "-" + strDateOut.substring(3,5);
...
function changeTotal (){
	...
	const surchargeAdult = document.getElementById('NumberAdult').value - (document.getElementById('NumberAdult').max - 2);
	const surchargeChild = document.getElementById('NumberChild').value - (document.getElementById('NumberChild').max - 2);
...
document.getElementById('ComeInDate').addEventListener('input', () => {
	changeTotal();
})

document.getElementById('ComeOutDate').addEventListener('input', () => {
	changeTotal();
})
...
function submitFindRoom(){	
	...
	document.querySelector('.submitFindByCondition').click();
}
...
function addInputTypeRoomName(TypeRoomName){
	...
	const inputTypeRoomName = document.createElement('input');
	inputTypeRoomName.className = "inputTypeRoomName";
	inputTypeRoomName.type = "text";
	inputTypeRoomName.name = "TypeRoomName";
	inputTypeRoomName.value = TypeRoomName;
    formContainer.appendChild(inputTypeRoomName);
}
